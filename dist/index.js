...
const main = async () => {
    ...
    const acceptRes = await org2PkgService.acceptTransfer(packageID, termsId, transferTerms);
    log.success(`Transfer accepted: ${acceptRes.id}`);
    // Update status after accept
    const acceptStatusRes = await org2PkgService.updateStatusAfterAccept(packageID, termsId);
