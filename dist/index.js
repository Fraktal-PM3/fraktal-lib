...
const main = async () => {
    ...
    try {
        ...
        const org1PermResult = await org1RoleAuthService.setPermissions("Org1MSP", [
            "package:create",
            "package:read",
        ...
        log.success(`Permissions assigned to Org1MSP - TX ID: ${org1PermResult.id}`);
    ...
    try {
        ...
        const org2PermResult = await org1RoleAuthService.setPermissions("Org2MSP", [
            "package:create",
            "package:read",
            "package:read:private",
            "package:updateStatus",
        ...
        log.success(`Permissions assigned to Org2MSP - TX ID: ${org2PermResult.id}`);
    ...
    if (!org1Perms || org1Perms.length === 0) {
        log.error("WARNING: Org1MSP permissions are empty! setPermissions may have failed silently.");
    }
    ...
    if (!org2Perms || org2Perms.length === 0) {
        log.error("WARNING: Org2MSP permissions are empty! setPermissions may have failed silently.");
    }
    ...
    const packageID3 = (0, crypto_1.randomUUID)();
    const salt3 = crypto_1.default.randomBytes(16).toString("hex");
    const createRes3 = await org1PkgService.createPackage(packageID3, packageDetails, pii, salt3);
    log.success(`Package created for deletion permission test: ${createRes3.id}`);
    ...
    const deleteRes2 = await org1PkgService
        ...
        .catch((e) => {
        log.info(`Expected error on deletion without permission: ${e.message}`);
    });
